...
- 增加播放模式，支持 单视频（播完即停）、单视频循环、合集/分P的顺序或逆序播放、UGC列表视频的顺序或逆序播放、推荐视频随机播放、自定义（按用户自己设置的策略顺序播放）
  - TV 视频列表入口（推荐、热门、动态、稍后再看、收藏、历史、UP 投稿、标签、分区、视频搜索、个人中心视频横排、详情相关推荐）在合集/分P模式下沿当前已加载的来源列表连续播放，从点击的视频位置开始；逆序模式同样适用。
  - 独立进入追番/追剧、PGC 首页、索引、时间线或番剧搜索时，按剧集播放，不沿用之前的视频列表。明确选择的单视频、单视频循环、自定义等模式保持原有语义。
